...
TM_EXTI_Result_t TM_EXTI_Attach(GPIO_TypeDef* GPIOx, uint16_t GPIO_Line, TM_EXTI_Trigger_t trigger, TM_GPIO_PuPd_t pull) {
	TM_GPIO_PuPd_t PuPd;
	...
	if (!(GPIO_Line && !(GPIO_Line & (GPIO_Line - 1)))) {
		...
		for (i = 0; i < 0x10; i++) {
			if (GPIO_Line & (1 << i)) {
				...
				if (TM_EXTI_Attach(GPIOx, 1 << i, trigger, pull) != TM_EXTI_Result_Ok) {
